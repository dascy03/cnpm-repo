...
const PrinterMana = (props) => {
    ...
    const handleQueueCLick = (printerID,model) => {
                        ...
                        if (key % 2 === 0) {
                            return (
                                <tr className="text-center text-xl bg-[#E8F6FD]" key={key} >
                                    <th className="h-12 w-1/12">{val.printerID}</th>
                                    <th className="w-2/12">{val.model}</th>
                                    <th className="w-2/12">{val.location}</th>
                                    <th className="w-2/12">{val.pageBalance}</th>
                                    <th className="w-2/12">
                                    {(val.status === "Đang hoạt động") ? <span className="text-green-500">{val.status}</span> : <span className="text-red-500">{val.status}</span>}
                                    </th>
                                    <th className="w-1/12"><button onClick={()=>handleStatusClick(val.printerID)}>{showEye(val.status)}</button></th>
                                    <th className="w-1/12"><button onClick={()=>handleQueueCLick(val.printerID,val.model)}><img src="/bars-solid.svg" className="h-7" alt="bars-solid" /></button></th>
                                    <th className="w-1/12"><button onClick={()=>handleDeleteClick(val)}><img src="/xmark.svg" className="h-8" alt="xmark"/></button></th>
                                </tr>
                            )
                        }
                        else {
